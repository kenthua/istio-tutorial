...
NOTE: There may be delays due to caching in istio proxy

== Role-Based Access Control
----
# Default is deny all once applied
istioctl create -f istiofiles/istio-rbac-enable.yaml

bin/poll_streamlined_customer.sh

# namespace-level
istioctl create -f istiofiles/istio-rbac-namespace.yaml

bin/poll_streamlined_customer.sh

istioctl delete -f istiofiles/istio-rbac-namespace.yaml

# service-level 

istioctl create -f istiofiles/istio-rbac-customer.yaml

istioctl create -f istiofiles/istio-rbac-preference.yaml

istioctl create -f istiofiles/istio-rbac-recommendation.yaml

bin/poll_streamlined_customer.sh

istioctl delete -f istiofiles/istio-rbac-customer.yaml

istioctl delete -f istiofiles/istio-rbac-preference.yaml

istioctl delete -f istiofiles/istio-rbac-recommendation.yaml

istioctl delete -f istiofiles/istio-rbac-enable.yaml
...
==== Test Circuit Breaker v1alpha3 - kenthua
This example is performed with a DestinationRule.  Previous istio examples used a RouteRule and a DestinationPolicy
 
[source,bash]
----
# Deploy a version of the node code that has timeout
# In TUTORIAL_HOME/recommendation/node
cp recommendation.timeout.js recommendation.js

# gcloud rebuild / deploy recommendation v2

# Deploy istio DestinationRule
istioctl create -f istiofiles/recommendation_cb_policy_alpha_3.yml -n tutorial

# cd to ISTIO_HOME
#
# Use fortio pod from istio example
kubectl apply -f samples/httpbin/sample-client/fortio-deploy.yaml -n tutorial

FORTIO_POD=$(kubectl get pod -n tutorial | grep fortio | awk '{ print $1 }')

# Run a single test
kubectl exec -it $FORTIO_POD -n tutorial -c fortio /usr/local/bin/fortio -- load -curl  http://recommendation:8080

# Add concurrency
kubectl exec -it $FORTIO_POD -n tutorial -c fortio /usr/local/bin/fortio -- load -c 2 -qps 0 -n 20 -loglevel Warning http://recommendation:8080

# Increase concurrency count from 2 to higher

# Check the logs
kubectl exec -it $FORTIO_POD -n tutorial -c istio-proxy -- sh -c 'curl localhost:15000/stats' | grep recommendation | grep pending

----

==== Alternative deployment with alpha 1, RouteRule + DestinationPolicy
