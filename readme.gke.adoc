= Java (Spring Boot and Vert.x) + Istio on Kubernetes/GKE
...
# increased liveness probe initial seconds from 10 to 20?  It was having issues deploying because it kept saying it was unhealthy
kubectl apply -f <(istioctl kube-inject -f ../../kubernetes/Deployment.yml) -n tutorial

# added type: LoadBalancer
kubectl apply -f ../../kubernetes/Service.yml -n tutorial
...
# Modified image location
# increased liveness probe initial seconds from 10 to 20?  It was having issues deploying 
kubectl apply -f <(istioctl kube-inject -f ../../kubernetes/Deployment.yml) -n tutorial

# added type: LoadBalancer
kubectl apply -f ../../kubernetes/Service.yml -n tutorial

watch kubectl get pods -n tutorial | grep preference
----
...
[source,bash]
----
URL=`kubectl get svc preference -n tutorial -o=jsonpath='{.status.loadBalancer.ingress[].ip}'`
curl $URL:8080
----
...
gcloud container builds submit --config cloudbuild-v1.yaml .
...
watch kubectl get pods -n tutorial | grep recommendation 
----
...
[source,bash]
----
URL=`kubectl get svc recommendation -n tutorial -o=jsonpath='{.status.loadBalancer.ingress[].ip}'`
curl $URL:8080
----
